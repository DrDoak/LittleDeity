	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata_t IN)
			{
				...
				//OUT.texcoord = float2(OUT.texcoord.x, OUT.texcoord.y% 1);
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				fixed4 c = tex2D(_GrabTexture, IN.grabcoord + (refraction * tex2D(_DistortionTexture, distortionScale * (IN.texcoord%1)))) * IN.color;
				c.rgb *= c.a;
