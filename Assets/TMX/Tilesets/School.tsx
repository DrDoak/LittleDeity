<?xml version="1.0" encoding="UTF-8"?>
<tileset name="School" tilewidth="32" tileheight="32" tilecount="100" columns="10">
 <image source="../../Art/Tiles/School.png" width="320" height="320"/>
 <terraintypes>
  <terrain name="floor" tile="0"/>
  <terrain name="stone_wall" tile="0"/>
  <terrain name="wood_floor" tile="0"/>
 </terraintypes>
 <tile id="0" terrain=",,,0"/>
 <tile id="1" terrain=",,0,0"/>
 <tile id="2" terrain=",,0,"/>
 <tile id="3" terrain=",,,1"/>
 <tile id="4" terrain=",,1,1"/>
 <tile id="5" terrain=",,1,"/>
 <tile id="6" terrain=",,,2"/>
 <tile id="7" terrain=",,2,2"/>
 <tile id="8" terrain=",,2,"/>
 <tile id="10" terrain=",0,,0"/>
 <tile id="11" terrain="0,0,0,0"/>
 <tile id="12" terrain="0,,0,"/>
 <tile id="13" terrain=",1,,1"/>
 <tile id="14" terrain="1,1,1,1"/>
 <tile id="15" terrain="1,,1,"/>
 <tile id="16" terrain=",2,,2"/>
 <tile id="17" terrain="2,2,2,2"/>
 <tile id="18" terrain="2,,2,"/>
 <tile id="20" terrain=",0,,"/>
 <tile id="21" terrain="0,0,,"/>
 <tile id="22" terrain="0,,,"/>
 <tile id="23" terrain=",1,,"/>
 <tile id="24" terrain="1,1,,"/>
 <tile id="25" terrain="1,,,"/>
 <tile id="26" terrain=",2,,"/>
 <tile id="27" terrain="2,2,,"/>
 <tile id="28" terrain="2,,,"/>
 <tile id="30" terrain="0,0,0,"/>
 <tile id="31" terrain="0,0,,0"/>
 <tile id="33" terrain="1,1,1,"/>
 <tile id="34" terrain="1,1,,1"/>
 <tile id="36" terrain="2,2,2,"/>
 <tile id="37" terrain="2,2,,2"/>
 <tile id="40" terrain="0,,0,0"/>
 <tile id="41" terrain=",0,0,0"/>
 <tile id="43" terrain="1,,1,1"/>
 <tile id="44" terrain=",1,1,1"/>
 <tile id="46" terrain="2,,2,2"/>
 <tile id="47" terrain=",2,2,2"/>
</tileset>
